...
export const readPropertyTestOptions = (source = process.env) => ({
...
export const readDatabaseTestConfig = (source = process.env) => {
    ...
    if (parsed.protocol !== 'postgres:' && parsed.protocol !== 'postgresql:') {
        throw new DatabaseTestConfigurationError('TEST_DATABASE_URL must use postgres:// or postgresql://');
    }
...
export const requireDatabaseTestConfig = (source = process.env) => {
    ...
    if (!configuredUrl) {
        throw new DatabaseTestConfigurationError('Set TEST_DATABASE_URL to an isolated PostgreSQL test database before running database tests');
    }
